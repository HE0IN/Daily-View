...
옛 `archived`(보관) 플래그를 **전부 해제**하는 정리가 **앱 시작 시 자동 실행**된다.
별도 조치 없이 위 절차대로 재시작하면 된다.

재시작 전에 무엇이 어디로 돌아가는지 미리 보려면 (백업 후):

```powershell
scripts\backup.bat
.venv\Scripts\python.exe scripts\migrate_deleted_tag.py --dry-run   # 미리보기
.venv\Scripts\python.exe scripts\migrate_deleted_tag.py             # 실제 복구
```

**옛 데이터는 삭제와 자동보관을 구분할 수 없다.** 완료 항목을 다시 열면
(완료 → 등록자검토중 → 반려) 보관 플래그는 남고 상태만 바뀌기 때문이다. 그래서
추측으로 삭제 표시를 하지 않고 **전부 원래 단계로 되살린다** — 각 항목은 자기
상태에 맞는 섹션으로 돌아간다 (완료 → 완료, 담당자검토중 → 담당자 처리,
확인대기 → 확인요청목록 …). 재실행해도 안전하다.

> **정리 후 할 일**: 지금 '삭제' 에 있던 항목이 전부 각 단계로 돌아온다. 그중
> 정말 지울 것은 **개발목록에서 카드 체크 → [🗑 N건 삭제]** 로 한 번에 처리한다
> (한 건씩은 상세보기 🗑). 이후로는 삭제 태그로 남아 다시 뒤섞이지 않는다.
> 잘못 지웠으면 '삭제 항목: 삭제만' 으로 보고 [↩ 복구].
